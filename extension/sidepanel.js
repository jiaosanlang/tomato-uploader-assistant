...
function parseChapterNumber(title) { const match = title.match(/^第\s*([0-9０-９一二三四五六七八九十百千万零〇两]+)\s*[章节回]/); if (!match) return 0; const digits = match[1].replace(/[０-９]/g, (char) => String.fromCharCode(char.charCodeAt(0) - 0xfee0)); if (/^\d+$/.test(digits)) return Number(digits); const map = { 零: 0, 〇: 0, 一: 1, 二: 2, 两: 2, 三: 3, 四: 4, 五: 5, 六: 6, 七: 7, 八: 8, 九: 9 }; let total = 0; let section = 0; for (const char of digits) { if ('十百千万'.includes(char)) { const unit = { 十: 10, 百: 100, 千: 1000, 万: 10000 }[char]; section = (section || 1) * unit; if (unit === 10000) { total += section; section = 0; } } else section += map[char] ?? 0; } return total + section; }
...
function appendChaptersFromText(chapters, fileName, text) {
  ...
  for (const raw of lines) { const line = raw.trim(); if (chapterPattern.test(line)) { push(); current = { title: line, number: parseChapterNumber(line), content: [], source: fileName }; } else if (current) current.content.push(raw); }
  push();
  if (!chapters.some((chapter) => chapter.source === fileName) && normalized) { const title = fileName.replace(/\.txt$/i, '').trim(); const content = removeLeadingDuplicateTitle(normalized, title); chapters.push({ title: cleanChapterTitle(title), number: parseChapterNumber(title), content, chars: (content.match(/[一-鿿]/g) || []).length, source: fileName }); }
}
